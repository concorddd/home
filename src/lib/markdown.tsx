...
export function parseMarkdown(text: string): React.ReactNode[] {
  ...
  for (let i = 0; i < lines.length; i++) {
      ...
      if (inCodeBlock) {
        ...
        elements.push(
          <pre key={`code-${i}`} className="my-1 rounded bg-[#1e1f22] p-3 overflow-x-auto">
            <code className="text-sm text-[#dbdee1] font-mono">{codeBlockContent.join("\n")}</code>
          </pre>
        );
        codeBlockContent = [];
        codeBlockLang = "";
    ...
    if (line.startsWith("> ")) {
      elements.push(
        <blockquote key={`quote-${i}`} className="my-1 border-l-4 border-[#4e5058] pl-3 text-[#dbdee1] italic">
...
function parseInlineMarkdown(text: string): React.ReactNode[] {
  ...
  while ((match = regex.exec(text)) !== null) {
    ...
    if (bold) {
      elements.push(
        <strong key={`b-${keyIndex++}`} className="font-bold text-[#dbdee1]">
          {bold}
        </strong>
      );
    } else if (italic1) {
      elements.push(
        <em key={`i-${keyIndex++}`} className="italic text-[#dbdee1]">
          {italic1}
        </em>
      );
    } else if (italic2) {
      ...
      elements.push(
        <del key={`s-${keyIndex++}`} className="line-through text-[#949ba4]">
          {strikethrough}
        </del>
      );
    } else if (code) {
