    ...
    return (
      <div className="border-b border-[#1e1f22] bg-[#2b2d31] p-4">
        <div className="flex items-center justify-between mb-2">
          <h3 className="text-sm font-semibold text-[#dbdee1]">Mensagens Fixadas</h3>
          <button onClick={onClose} className="text-[#949ba4] hover:text-white">
            ✕
          </button>
        </div>
        <p className="text-sm text-[#949ba4]">Nenhuma mensagem fixada.</p>
      </div>
    );
  }
      ...
      <div className="space-y-2">
        {pinnedMessages.map((msg) => (
          <div key={msg.id} className="rounded bg-[#1e1f22] p-2">
            <p className="text-sm text-[#dbdee1] line-clamp-2">{msg.content}</p>
