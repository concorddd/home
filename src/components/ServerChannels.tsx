    ...
    >
      {items.map((item, i) =>
        "separator" in item && item.separator ? (
          <div key={`sep-${i}`} className="mx-2 my-1 h-px bg-[#2b2d31]" />
        ) : (
          <button
            key={`item-${i}`}
            role="menuitem"
            ...
            className={cn(
              ...
              item.noHoverStyle
                ...
                : item.danger
                  ? "text-[#dbdee1] hover:bg-[#da373c] hover:text-white"
                  : "text-[#949ba4] hover:bg-[#5865F2] hover:text-white",
            )}
          >
            ...
            {typeof item.checkbox === "boolean" && (
              <span
                aria-hidden
                className={cn(
                  "flex size-[18px] shrink-0 items-center justify-center rounded-[3px] border-2 border-[#949ba4]",
