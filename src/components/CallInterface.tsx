        ...
        <div className={`flex flex-wrap justify-center gap-3 max-w-md ${participants.length > 4 ? "gap-2" : "gap-3"}`}>
          {participants.map((p) => (
            <div key={p.id} className="flex flex-col items-center gap-1">
              ...
              <span className="text-[11px] text-[#dbdee1] truncate max-w-[60px]">{p.display_name || p.username}</span>
            </div>
          ))}
        </div>
        <div className="mt-3 text-sm text-[#949ba4]">{formatDuration(duration)}</div>
      </div>
...
function ControlButton({ active, onClick, title, children }: { active: boolean; onClick: () => void; title: string; children: React.ReactNode }) {
  return (
    <button onClick={onClick} title={title} className={`flex size-11 items-center justify-center rounded-full transition-colors ${active ? "bg-[#404249] text-white hover:bg-[#4e5058]" : "bg-[#404249] text-[#949ba4] hover:bg-[#404249] hover:text-[#dbdee1]"}`}>
      {children}
    </button>
  );
}
